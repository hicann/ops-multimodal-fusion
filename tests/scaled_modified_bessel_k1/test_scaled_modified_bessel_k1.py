...
if not hasattr(torch.ops.ops_multimodal_fusion, "scaled_modified_bessel_k1"):
    pytest.skip(
        "ops_multimodal_fusion.scaled_modified_bessel_k1 not registered for current NPU_ARCH; skipping module",
...
def run_op(x: torch.Tensor) -> torch.Tensor:
    return torch.ops.ops_multimodal_fusion.scaled_modified_bessel_k1(x.npu()).cpu()
...
def test_scaled_modified_bessel_k1_interface_exist():
    assert hasattr(torch.ops.ops_multimodal_fusion, "scaled_modified_bessel_k1")


@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_scaled_modified_bessel_k1_empty():
...
def test_scaled_modified_bessel_k1_regular_random(shape):
...
def test_scaled_modified_bessel_k1_regbase_2d_shapes(shape):
...
def test_scaled_modified_bessel_k1_regbase_3d_shapes(shape):
    x = torch.rand(*shape, dtype=torch.float32) * 32.0 + 1e-6
    expected = reference(x)
    out = run_op(x)
    assert out.shape == x.shape
...
def test_scaled_modified_bessel_k1_regbase_4d_shapes(shape):
    x = torch.rand(*shape, dtype=torch.float32) * 32.0 + 1e-6
    expected = reference(x)
    out = run_op(x)
    assert out.shape == x.shape
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_scaled_modified_bessel_k1_boundary_samples():
    x = torch.tensor(
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_scaled_modified_bessel_k1_dense_piecewise_ranges():
    x = torch.cat(
        [
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_scaled_modified_bessel_k1_non_contiguous_input():
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_scaled_modified_bessel_k1_special_values():
    x = torch.tensor(
