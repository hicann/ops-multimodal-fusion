...
if not hasattr(torch.ops.ops_multimodal_fusion, "shifted_chebyshev_polynomial_v"):
    pytest.skip(
        "ops_multimodal_fusion.shifted_chebyshev_polynomial_v not registered for current NPU_ARCH; skipping module",
...
def run_op(x: torch.Tensor, n: torch.Tensor) -> torch.Tensor:
    return torch.ops.ops_multimodal_fusion.shifted_chebyshev_polynomial_v(x.npu(), n.npu()).cpu()
...
def test_shifted_chebyshev_polynomial_v_interface_exist():
    assert hasattr(torch.ops.ops_multimodal_fusion, "shifted_chebyshev_polynomial_v")


@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_shifted_chebyshev_polynomial_v_empty():
...
def test_shifted_chebyshev_polynomial_v_regular_random(shape, degree):
...
def test_shifted_chebyshev_polynomial_v_regbase_2d_shapes(shape):
    x = torch.rand(*shape, dtype=torch.float32) * 2.0
...
def test_shifted_chebyshev_polynomial_v_regbase_3d_shapes(shape):
    x = torch.rand(*shape, dtype=torch.float32) * 2.0
    n = torch.full(shape, 17.0, dtype=torch.float32)
    expected = reference(x, n)
    out = run_op(x, n)
...
def test_shifted_chebyshev_polynomial_v_regbase_4d_shapes(shape):
    x = torch.rand(*shape, dtype=torch.float32) * 2.0
    n = torch.full(shape, 17.0, dtype=torch.float32)
    expected = reference(x, n)
    out = run_op(x, n)
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_shifted_chebyshev_polynomial_v_broadcast():
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_shifted_chebyshev_polynomial_v_boundary_samples():
    x = torch.tensor(
        [
            -0.5,
            -1e-4,
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_shifted_chebyshev_polynomial_v_non_contiguous_inputs():
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_shifted_chebyshev_polynomial_v_exact_endpoints():
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_shifted_chebyshev_polynomial_v_fractional_n_matches_truncation():
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_shifted_chebyshev_polynomial_v_recurrence_dense_interval():
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_shifted_chebyshev_polynomial_v_large_shifted_abs_x_high_degree():
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_shifted_chebyshev_polynomial_v_special_values():
