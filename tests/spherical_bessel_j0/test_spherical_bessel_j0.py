...
if not hasattr(torch.ops.ops_multimodal_fusion, "spherical_bessel_j0"):
    pytest.skip(
        "ops_multimodal_fusion.spherical_bessel_j0 not registered for current NPU_ARCH; skipping module",
...
def run_op(x: torch.Tensor) -> torch.Tensor:
    return torch.ops.ops_multimodal_fusion.spherical_bessel_j0(x.npu()).cpu()
...
def test_spherical_bessel_j0_interface_exist():
    assert hasattr(torch.ops.ops_multimodal_fusion, "spherical_bessel_j0")


@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_spherical_bessel_j0_empty():
...
def test_spherical_bessel_j0_regular_random(shape):
    x = (torch.rand(*shape, dtype=torch.float32) - 0.5) * 64.0
    expected = reference(x)
    out = run_op(x)
    assert out.shape == x.shape
    assert out.dtype == torch.float32
    assert_close(out, expected, rtol=6e-5, atol=6e-5)


@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
@pytest.mark.parametrize("shape", SHAPES_2D)
def test_spherical_bessel_j0_regbase_2d_shapes(shape):
    x = (torch.rand(*shape, dtype=torch.float32) - 0.5) * 64.0
    expected = reference(x)
    out = run_op(x)
    assert out.shape == x.shape
...
def test_spherical_bessel_j0_regbase_3d_shapes(shape):
    x = (torch.rand(*shape, dtype=torch.float32) - 0.5) * 64.0
    expected = reference(x)
    out = run_op(x)
    assert out.shape == x.shape
...
def test_spherical_bessel_j0_regbase_4d_shapes(shape):
    x = (torch.rand(*shape, dtype=torch.float32) - 0.5) * 64.0
    expected = reference(x)
    out = run_op(x)
    assert out.shape == x.shape
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_spherical_bessel_j0_boundary_samples():
    x = torch.tensor(
        [
            -32.0,
            -16.0,
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_spherical_bessel_j0_dense_small_interval():
    x = torch.linspace(-0.6, 0.6, steps=2048, dtype=torch.float32)
    expected = reference(x)
    out = run_op(x)
    assert_close(out, expected, rtol=6e-5, atol=6e-5)
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_spherical_bessel_j0_non_contiguous_input():
    base = (torch.rand(9, 17, dtype=torch.float32) - 0.5) * 20.0
    x = base[:, ::2]
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_spherical_bessel_j0_special_values():
    x = torch.tensor(
    ...
    assert_close(out[2:4], reference(x[2:4]), rtol=6e-5, atol=6e-5)
