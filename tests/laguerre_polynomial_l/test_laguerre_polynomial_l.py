...
if not hasattr(torch.ops.ops_multimodal_fusion, "laguerre_polynomial_l"):
    pytest.skip(
        "ops_multimodal_fusion.laguerre_polynomial_l not registered for current NPU_ARCH; skipping module",
...
def reference_manual(x: torch.Tensor, n: torch.Tensor) -> torch.Tensor:
    ...
    neg_mask = n_int < 0
    abs_zero_mask = torch.abs(x) == 0.0
    zero_mask = n_int == 0
    one_mask = n_int == 1
    recur_mask = ~(nan_mask | neg_mask | abs_zero_mask | zero_mask | one_mask)

    out[nan_mask] = torch.nan
    out[neg_mask] = 0.0
    out[abs_zero_mask] = 1.0
    out[zero_mask] = 1.0
    out[one_mask] = 1.0 - x[one_mask]
    ...
    if recur_mask.any():
        ...
        for k in range(1, int(n_sel.max().item())):
            active = (k < n_sel) & ~torch.isnan(q)
            if active.any():
                coeff = (float(k + k)) + (1.0 - x_sel[active])
                r_new = (coeff * q[active] - float(k) * p[active]) / float(k + 1)
                r[active] = r_new
                p[active] = q[active]
                q[active] = r_new
        out[recur_mask] = r
...
def reference(x: torch.Tensor, n: torch.Tensor) -> torch.Tensor:
    ...
    return torch.special.laguerre_polynomial_l(x, n_norm)


def run_op(x: torch.Tensor, n: torch.Tensor) -> torch.Tensor:
    return torch.ops.ops_multimodal_fusion.laguerre_polynomial_l(x.npu(), n.npu()).cpu()
...
def test_laguerre_polynomial_l_interface_exist():
    assert hasattr(torch.ops.ops_multimodal_fusion, "laguerre_polynomial_l")


@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_laguerre_polynomial_l_empty():
...
@pytest.mark.parametrize("degree", [-3.0, -1.0, 0.0, 1.0, 2.0, 5.0, 9.0, 17.0, 33.0])
def test_laguerre_polynomial_l_regular_random(shape, degree):
    x = (torch.rand(*shape, dtype=torch.float32) - 0.5) * 8.0
...
def test_laguerre_polynomial_l_regbase_2d_shapes(shape):
    x = (torch.rand(*shape, dtype=torch.float32) - 0.5) * 8.0
...
def test_laguerre_polynomial_l_regbase_3d_shapes(shape):
    x = (torch.rand(*shape, dtype=torch.float32) - 0.5) * 8.0
    n = torch.full(shape, 17.0, dtype=torch.float32)
    expected = reference(x, n)
    out = run_op(x, n)
...
def test_laguerre_polynomial_l_regbase_4d_shapes(shape):
    x = (torch.rand(*shape, dtype=torch.float32) - 0.5) * 8.0
    n = torch.full(shape, 17.0, dtype=torch.float32)
    expected = reference(x, n)
    out = run_op(x, n)
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_laguerre_polynomial_l_broadcast():
    x = (torch.rand(3, 4, 5, dtype=torch.float32) - 0.5) * 6.0
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_laguerre_polynomial_l_special_cases():
    x = torch.tensor(
        [-2.0, -1.5, -0.0, 0.0, 0.25, 1.0, float("nan"), float("inf"), float("-inf")],
        dtype=torch.float32,
    )
    n = torch.tensor(
        [-3.0, -1.0, 7.0, 5.0, 1.0, 2.0, 3.0, 2.0, 3.0],
        dtype=torch.float32,
    )
    expected = reference(x, n)
    out = run_op(x, n)
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_laguerre_polynomial_l_non_contiguous_inputs():
    base_x = (torch.rand(9, 17, dtype=torch.float32) - 0.5) * 6.0
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_laguerre_polynomial_l_fractional_n_matches_truncation():
    x = torch.tensor([-3.0, -1.0, 0.25, 1.5, 3.0], dtype=torch.float32)
    n = torch.tensor([7.9, 6.2, 1.9, -2.7, 18.8], dtype=torch.float32)
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_laguerre_polynomial_l_exact_small_orders():
    x = torch.tensor([-2.0, -1.0, 0.0, 1.0, 2.0], dtype=torch.float32)
    n = torch.tensor([0.0, 1.0, 2.0, 3.0, 4.0], dtype=torch.float32)
    expected = torch.tensor([1.0, 2.0, 1.0, -0.6666667, 0.33333334], dtype=torch.float32)
    out = run_op(x, n)
    assert_close(out, expected, rtol=0.0, atol=1e-6)


@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_laguerre_polynomial_l_dense_manual_recurrence():
    x = torch.linspace(-4.0, 4.0, steps=4096, dtype=torch.float32)
