...
if not hasattr(torch.ops.ops_multimodal_fusion, "add"):
    pytest.skip(
        "ops_multimodal_fusion.add not registered for current NPU_ARCH; skipping module",
        allow_module_level=True,
    )


def test_add_interface_exist():
    """Test that the 'ops_multimodal_fusion.add' operator is registered in torch.ops."""
    assert hasattr(torch.ops.ops_multimodal_fusion, "add"),\
        "The 'add' operator is not registered in the 'torch.ops.ops_multimodal_fusion' namespace."
...
def test_add_operator(shape, dtype):
    """
    Test the add operator with various shapes and data types.
    ...
    a_npu = a.npu()
    b_npu = b.npu()
    result_npu = torch.ops.ops_multimodal_fusion.add(a_npu, b_npu)
