...
if not hasattr(torch.ops.ops_multimodal_fusion, "modified_bessel_k0"):
    pytest.skip(
        "ops_multimodal_fusion.modified_bessel_k0 not registered for current NPU_ARCH; skipping module",
...
def run_op(x: torch.Tensor) -> torch.Tensor:
    return torch.ops.ops_multimodal_fusion.modified_bessel_k0(x.npu()).cpu()


def assert_close(actual: torch.Tensor, expected: torch.Tensor, *, rtol=2e-5, atol=2e-5):
...
def test_modified_bessel_k0_interface_exist():
    assert hasattr(torch.ops.ops_multimodal_fusion, "modified_bessel_k0")


@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_modified_bessel_k0_empty():
...
def test_modified_bessel_k0_regular_random(shape):
...
def test_modified_bessel_k0_regbase_2d_shapes(shape):
...
def test_modified_bessel_k0_regbase_3d_shapes(shape):
    x = torch.rand(*shape, dtype=torch.float32) * 32.0 + 1e-6
    expected = reference(x)
    out = run_op(x)
    assert out.shape == x.shape
...
def test_modified_bessel_k0_regbase_4d_shapes(shape):
    x = torch.rand(*shape, dtype=torch.float32) * 32.0 + 1e-6
    expected = reference(x)
    out = run_op(x)
    assert out.shape == x.shape
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_modified_bessel_k0_non_contiguous_input():
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_modified_bessel_k0_boundary_samples():
    x = torch.tensor(
        [
            1e-12,
            1e-8,
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_modified_bessel_k0_special_values():
    x = torch.tensor(
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_modified_bessel_k0_no_nan_or_inf_on_positive_finite_inputs():
    x = torch.cat(
        [
