...
if not hasattr(torch.ops.ops_multimodal_fusion, "chebyshev_polynomial_t"):
    pytest.skip(
        "ops_multimodal_fusion.chebyshev_polynomial_t not registered for current NPU_ARCH; skipping module",
...
def run_op(x: torch.Tensor, n: torch.Tensor) -> torch.Tensor:
    return torch.ops.ops_multimodal_fusion.chebyshev_polynomial_t(x.npu(), n.npu()).cpu()
...
def test_chebyshev_polynomial_t_interface_exist():
    assert hasattr(torch.ops.ops_multimodal_fusion, "chebyshev_polynomial_t")


@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_t_empty():
...
def test_chebyshev_polynomial_t_regular_random(shape, degree):
...
def test_chebyshev_polynomial_t_regbase_2d_shapes(shape):
    x = (torch.rand(*shape, dtype=torch.float32) - 0.5) * 4.0
...
def test_chebyshev_polynomial_t_regbase_3d_shapes(shape):
    x = (torch.rand(*shape, dtype=torch.float32) - 0.5) * 4.0
    n = torch.full(shape, 17.0, dtype=torch.float32)
    expected = reference(x, n)
    out = run_op(x, n)
...
def test_chebyshev_polynomial_t_regbase_4d_shapes(shape):
    x = (torch.rand(*shape, dtype=torch.float32) - 0.5) * 4.0
    n = torch.full(shape, 17.0, dtype=torch.float32)
    expected = reference(x, n)
    out = run_op(x, n)
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_t_broadcast():
    x = (torch.rand(3, 4, 5, dtype=torch.float32) - 0.5) * 2.4
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_t_boundary_samples():
    x_values = (-1.5, -1.0001, -1.0, -0.9999, -0.75, -1e-4, -0.0)
    x_values += (0.0, 1e-4, 0.75, 0.9999, 1.0, 1.0001, 1.5)
    n_values = (-3.0, 7.0, 8.0, 9.0, 6.0, 7.0, 0.0)
    n_values += (1.0, 2.0, 7.0, 8.0, 9.0, 10.0, 11.0)
    x = torch.tensor(x_values, dtype=torch.float32)
    n = torch.tensor(n_values, dtype=torch.float32)
    expected = reference(x, n)
    out = run_op(x, n)
    assert_close(out, expected)
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_t_dense_inner_interval():
    x = torch.linspace(-0.999, 0.999, steps=4096, dtype=torch.float32)
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_t_non_contiguous_inputs():
    base_x = (torch.rand(9, 17, dtype=torch.float32) - 0.5) * 3.0
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_t_special_values():
    x = torch.tensor(
        [-1.0, 1.0, float("nan"), float("inf"), float("-inf"), 0.5, -0.5],
        dtype=torch.float32,
    )
    n = torch.tensor(
        [3.0, 4.0, 5.0, 2.0, 3.0, -1.0, 8.0],
        dtype=torch.float32,
    )
    expected = reference(x, n)
    out = run_op(x, n)
    ...
    assert torch.isinf(out[3])
    assert torch.isnan(out[4])


@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_t_abs_one_exact_values():
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_t_large_abs_x_high_degree():
    x = torch.tensor([-20.0, -16.0, -15.0, 15.0, 16.0, 20.0], dtype=torch.float32)
    n = torch.tensor([7.0, 8.0, 9.0, 7.0, 8.0, 9.0], dtype=torch.float32)
    expected = reference(x, n)
    out = run_op(x, n)
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_t_negative_inner_interval_n7():
    x = torch.linspace(-0.999, -0.001, steps=2048, dtype=torch.float32)
    n = torch.full_like(x, 7.0)
    expected = reference(x, n)
    out = run_op(x, n)
    assert_close(out, expected, rtol=3e-4, atol=3e-4)
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_t_near_abs_one_high_degree():
    x = torch.tensor(
        [-0.999999, -0.9999, -0.999, 0.999, 0.9999, 0.999999],
        dtype=torch.float32,
    )
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_t_fractional_n_matches_truncation():
    x = torch.tensor([-0.8, -0.2, 0.2, 0.8], dtype=torch.float32)
