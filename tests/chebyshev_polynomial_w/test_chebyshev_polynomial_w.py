...
if not hasattr(torch.ops.ops_multimodal_fusion, "chebyshev_polynomial_w"):
    pytest.skip(
        "ops_multimodal_fusion.chebyshev_polynomial_w not registered for current NPU_ARCH; skipping module",
...
def run_op(x: torch.Tensor, n: torch.Tensor) -> torch.Tensor:
    return torch.ops.ops_multimodal_fusion.chebyshev_polynomial_w(x.npu(), n.npu()).cpu()
...
def test_chebyshev_polynomial_w_interface_exist():
    assert hasattr(torch.ops.ops_multimodal_fusion, "chebyshev_polynomial_w")


@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_w_empty():
...
def test_chebyshev_polynomial_w_regular_random(shape, degree):
...
def test_chebyshev_polynomial_w_regbase_2d_shapes(shape):
    x = (torch.rand(*shape, dtype=torch.float32) - 0.5) * 4.0
...
def test_chebyshev_polynomial_w_regbase_3d_shapes(shape):
    x = (torch.rand(*shape, dtype=torch.float32) - 0.5) * 4.0
    n = torch.full(shape, 17.0, dtype=torch.float32)
    expected = reference(x, n)
    out = run_op(x, n)
...
def test_chebyshev_polynomial_w_regbase_4d_shapes(shape):
    x = (torch.rand(*shape, dtype=torch.float32) - 0.5) * 4.0
    n = torch.full(shape, 17.0, dtype=torch.float32)
    expected = reference(x, n)
    out = run_op(x, n)
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_w_broadcast():
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_w_boundary_samples():
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_w_non_contiguous_inputs():
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_w_abs_one_exact_values():
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_w_fractional_n_matches_truncation():
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_w_recurrence_dense_inner_interval():
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_w_large_abs_x_high_degree():
