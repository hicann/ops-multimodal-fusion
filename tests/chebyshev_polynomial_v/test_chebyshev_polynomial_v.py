...
if not hasattr(torch.ops.ops_multimodal_fusion, "chebyshev_polynomial_v"):
    pytest.skip(
        "ops_multimodal_fusion.chebyshev_polynomial_v not registered for current NPU_ARCH; skipping module",
        allow_module_level=True,
    )


def reference(x: torch.Tensor, n: torch.Tensor) -> torch.Tensor:
    return torch.special.chebyshev_polynomial_v(x, n)


def run_op(x: torch.Tensor, n: torch.Tensor) -> torch.Tensor:
    return torch.ops.ops_multimodal_fusion.chebyshev_polynomial_v(x.npu(), n.npu()).cpu()
...
def test_chebyshev_polynomial_v_interface_exist():
    assert hasattr(torch.ops.ops_multimodal_fusion, "chebyshev_polynomial_v")


@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_v_empty():
...
def test_chebyshev_polynomial_v_regular_random(shape, degree):
...
def test_chebyshev_polynomial_v_regbase_2d_shapes(shape):
    x = (torch.rand(*shape, dtype=torch.float32) - 0.5) * 4.0
...
def test_chebyshev_polynomial_v_regbase_3d_shapes(shape):
    x = (torch.rand(*shape, dtype=torch.float32) - 0.5) * 4.0
    n = torch.full(shape, 17.0, dtype=torch.float32)
    expected = reference(x, n)
    out = run_op(x, n)
...
def test_chebyshev_polynomial_v_regbase_4d_shapes(shape):
    x = (torch.rand(*shape, dtype=torch.float32) - 0.5) * 4.0
    n = torch.full(shape, 17.0, dtype=torch.float32)
    expected = reference(x, n)
    out = run_op(x, n)
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_v_broadcast():
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_v_boundary_samples():
    x_values = (-1.5, -1.0001, -1.0, -0.9999, -0.75, -1e-4)
    x_values += (0.0, 1e-4, 0.75, 0.9999, 1.0, 1.0001, 1.5)
    n_values = (-3.0, 7.0, 8.0, 9.0, 6.0, 7.0)
    n_values += (0.0, 1.0, 2.0, 7.0, 9.0, 10.0, 11.0)
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_v_non_contiguous_inputs():
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_v_abs_one_exact_values():
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_v_fractional_n_matches_truncation():
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_v_recurrence_dense_inner_interval():
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_chebyshev_polynomial_v_large_abs_x_high_degree():
