...
if not hasattr(torch.ops.ops_multimodal_fusion, "scaled_modified_bessel_k0"):
    pytest.skip(
        "ops_multimodal_fusion.scaled_modified_bessel_k0 not registered for current NPU_ARCH; skipping module",
...
def run_op(x: torch.Tensor) -> torch.Tensor:
    return torch.ops.ops_multimodal_fusion.scaled_modified_bessel_k0(x.npu()).cpu()
...
def test_scaled_modified_bessel_k0_interface_exist():
    assert hasattr(torch.ops.ops_multimodal_fusion, "scaled_modified_bessel_k0")


@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_scaled_modified_bessel_k0_empty():
...
def test_scaled_modified_bessel_k0_regular_random(shape):
...
def test_scaled_modified_bessel_k0_regbase_2d_shapes(shape):
...
def test_scaled_modified_bessel_k0_regbase_3d_shapes(shape):
    x = torch.rand(*shape, dtype=torch.float32) * 32.0 + 1e-6
    expected = reference(x)
    out = run_op(x)
    assert out.shape == x.shape
...
def test_scaled_modified_bessel_k0_regbase_4d_shapes(shape):
    x = torch.rand(*shape, dtype=torch.float32) * 32.0 + 1e-6
    expected = reference(x)
    out = run_op(x)
    assert out.shape == x.shape
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_scaled_modified_bessel_k0_boundary_samples():
    x = torch.tensor(
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_scaled_modified_bessel_k0_dense_piecewise_ranges():
    x = torch.cat(
        [
            torch.linspace(1e-6, 2.0, steps=1024, dtype=torch.float32),
            torch.linspace(3.0, 64.0, steps=1024, dtype=torch.float32),
        ]
    )
    expected = reference(x)
    out = run_op(x)
    assert_close(out, expected, rtol=4e-5, atol=4e-5)


@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_scaled_modified_bessel_k0_non_contiguous_input():
...
@pytest.mark.skipif(not torch.npu.is_available(), reason="NPU device not found")
def test_scaled_modified_bessel_k0_special_values():
    x = torch.tensor(
