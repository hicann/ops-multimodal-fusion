...
# 检查无许可证头
grep -L "Copyright (c)" $(find applications -name "*.asc" -o -name "*.h")
# 检查公网 URL（常见域名）
...
for f in $(find applications -name "*.asc" -o -name "*.h"); do
  awk '/^using / {u=NR; next} /^#include / && u {print FILENAME ":" NR ": using before include"; exit}' "$f"
done

# 检查 extern "C" 中 #include
grep -B5 "^#include" $(find applications -name "*.asc" -o -name "*.h") | grep -B5 'extern "C"'
```
...
**grep 快速扫描**：
```bash
grep -nE "[^a-zA-Z_](/|%)[ ]*[a-zA-Z_]+" $(find applications -name "*.asc") | grep -v "_s"
```
...
grep -rn "\brealloc\b|\balloca\b" $(find applications -name "*.asc" -o -name "*.h")
```
...
**快速检查命令**：
```bash
grep -rEn "\b(exit|_exit|abort|atexit|kill|TerminateProcess)\b" $(find applications -name "*.asc")
```
...
**许可证头核对**（本仓无 OAT 脚本，直接对改动算子的 `.asc` / 同目录头文件手动核对）：
```bash
# 逐个改动算子核对（{op} 替换为算子名）
grep -L "Copyright (c)" applications/llm/{op}/arch35/*.asc applications/llm/{op}/arch35/*.h 2>/dev/null
```
