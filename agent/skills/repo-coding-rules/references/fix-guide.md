...
| **G.PRE.07** 宏名与关键字冲突 | 重命名宏，添加前缀或使用宏名规范（如 `OMF_`） |
...
| **G.AST.03** 断言检测运行时错误 | 替换为 `TORCH_CHECK(cond, "...")` 参数校验 |
...
| **CQ.07** 重复代码块 | 提取为公共函数，放同目录头文件（如 `applications/llm/{op}/arch35/{op}_common.h`） |
| **CQ.08** 重复文件 | 合并为同一文件 |
...
TORCH_CHECK(coreNum > 0, "GetCoreNumAiv failed, coreNum must be positive.");
int64_t perCore = totalN / coreNum;   // ← 此时 coreNum 必不为 0
```
...
TORCH_CHECK(ret == 0, "strncpy_s failed with ", ret);
```
