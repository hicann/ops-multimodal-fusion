...
# Modified by Huawei Technologies Co., Ltd. for Ascend NPU adaptation.
...
class _Voxelization(Function):
    ...
    def forward(
        ...
        is_npu = points.device.type == 'npu'
        orig_device = points.device
        if is_npu:
            points = points.cpu()
        voxel_size_t = torch.tensor(voxel_size, dtype=torch.float)
        coors_range_t = torch.tensor(coors_range, dtype=torch.float)
        if max_points == -1 or max_voxels == -1:
            coors = points.new_zeros(size=(points.size(0), 3), dtype=torch.int)
            ext_module.dynamic_voxelize_forward(
                points, voxel_size_t, coors_range_t, coors, NDim=3)
            if is_npu:
                coors = coors.to(orig_device)
            return coors
        else:
            ...
            ext_module.hard_voxelize_forward(
                points, voxel_size_t, coors_range_t,
                voxels, coors, num_points_per_voxel, voxel_num,
                max_points=max_points, max_voxels=max_voxels,
                NDim=3, deterministic=deterministic)
            voxels_out = voxels[:voxel_num]
            coors_out = coors[:voxel_num]
            num_points_per_voxel_out = num_points_per_voxel[:voxel_num]
            if is_npu:
                voxels_out = voxels_out.to(orig_device)
                coors_out = coors_out.to(orig_device)
                num_points_per_voxel_out = num_points_per_voxel_out.to(orig_device)
