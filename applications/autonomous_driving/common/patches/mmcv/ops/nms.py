...
def nms(boxes: array_like_type,
    ...
    # Ascend950PR CANN nms 算子不支持 (PTA feature not supported)，NPU 上回退 CPU
    orig_device = None
    if boxes.device.type in ('npu', 'mlu'):
    ...
    if orig_device is not None:
        dets = dets.to(orig_device)
        inds = inds.to(orig_device)
    if is_numpy:
        dets = dets.cpu().numpy()
        inds = inds.cpu().numpy()
    return dets, inds
...
def nms_rotated(dets: Tensor,
    ...
    if orig_device is not None:
        dets = dets.to(orig_device)
        keep_inds = keep_inds.to(orig_device)
