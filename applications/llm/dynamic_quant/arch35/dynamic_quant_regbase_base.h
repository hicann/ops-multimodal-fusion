 ...
 * \brief
 */
#ifndef DYNAMIC_QUANT_STORY_REGBASE_BASE_H
#define DYNAMIC_QUANT_STORY_REGBASE_BASE_H

#include "dynamic_quant_local_deps.h"
...
class DynamicQuantBase {
 public:
  ...
  __aicore__ inline void InitParams(GM_ADDR offset) {
    ...
    // headCore的每次UB搬运处理的row数量multiRowNum，headCore的UB搬运次数loopCnt，headCore常规搬运结后处理的row数量remainRow
    ...
    // 如果有tailCore的
    // tailCore的每次UB搬运处理的row数量multiRowNum，tailCore的UB搬运次数loopCnt，tailCore常规搬运结后处理的row数量remainRow
  ...
  __aicore__ inline void GetScale(float max_value, float& scale) {
    if (max_value == 0.0F) {
      max_value = DYNAMIC_QUANT_EPSINON;
    }
    if (ORIG_DTYPE_Y == DT_INT4) {
  ...
  __aicore__ inline void GetScaleAndOffset(float max_value, float min_value, float& scale, float& offset) {
    if (ORIG_DTYPE_Y == DT_INT4) {
      ...
      offset = DYNAMIC_QUANT_INT4_OFFSET;
    } else {
      scale = GetMax((max_value - min_value) / DYNAMIC_QUANT_INT8_SCALE, DYNAMIC_QUANT_EPSINON);
      offset = DYNAMIC_QUANT_INT8_OFFSET;
    }
    if (scale == 0.0F) {
      scale = DYNAMIC_QUANT_EPSINON;
    }
    offset -= max_value / scale;
  }
...
#endif  // DYNAMIC_QUANT_STORY_REGBASE_BASE_H
