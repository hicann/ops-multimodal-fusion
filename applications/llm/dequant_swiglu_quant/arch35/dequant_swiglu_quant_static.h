...
class DequantSwigluQuantBaseStatic {
 public:
  static constexpr int kStaticVariantTag = 1;
  ...
  static constexpr int kStaticVariantTagBias = 1;
  // bias数据类型为int32标记
  ...
  static constexpr int kStaticVariantTagX = 1;
  // x数据类型为int32标记
  ...
  static constexpr int kStaticVariantTagY = 1;
  // y数据类型为int8标记
 ...
 protected:
  ...
  // input global mem
  GlobalTensor<TXtype> xGm_; // 当前模支持 fp16, bf16, 用模参数去承载输入x的类型
  GlobalTensor<float> weightScaleGm_;
  GlobalTensor<TActScale> activationScaleGm_;
  GlobalTensor<TBias> biasGm_;  // 当前模支持 float，float16，bf16，int32， 用模参数去承载bias的类型
...
__aicore__ inline void DequantSwigluQuantBaseStatic<TActScale, TQuantScale, TGroup, TBias, TXtype, TYtype>::ProcessSingleGroupPerCore(int64_t groupIdx, int64_t xDimPerCore, int64_t coreDimxOffset)
{
    ...
    for (uint64_t i = 0; i < ubDimxLoop; i++) {
      // 核内循环每次计算时的起始地：当前核计算时的起始地 + 第i次核内循环要处理的起始地 + 已处理过的行偏移(历史已处理过的总行数)
      ...
      if (tl_->swiGluMode == 0) {
        ...
        // 奇偶搬运，连续搬运左右字，actOffset和gateOffset不生效
        DataCopyParams dataCopyXParams;
        dataCopyXParams.blockCount = xDimPerLoop;
