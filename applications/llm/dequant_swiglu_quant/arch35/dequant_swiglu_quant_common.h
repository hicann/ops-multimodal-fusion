...
__aicore__ inline void FloatDequant(__local_mem__ TXtype* xPtr, __local_mem__ float* dstPtr,
  ...
  /*
  输入xint32类型，参数weightScale、activationScale、bias不存在，直接对x做类型转换
  */
...
__aicore__ inline void SwigluV2SingleYWithQuantScale(__ubuf__ float* x1Ptr,
  ...
  // 根据repeatTime的循环次数判断尾需要处理的mask
  // 如果repeatTime是偶数，则尾的mask是：(64 + tailCount) / 2；奇数的，则尾的mask是：tailCount / 2
  if ((repeatTimes & 1) == 0) {
    // repeatTimes是偶数
