...
class DequantSwigluQuantNlast {
 public:
  static constexpr int kNlastVariantTag = 1;
  ...
  static constexpr int kNlastVariantTagBias = 1;
  // bias数据类型为int32标记
  ...
  static constexpr int kNlastVariantTagX = 1;
  // x数据类型为int32标记
  ...
  static constexpr int kNlastVariantTagY = 1;
  // y数据类型为int8标记
  ...
  __VEC_SCOPE__
  {
    ...
    for (uint16_t j = 0; j < repeatTimes; j++) {
        ...
        for (uint16_t i = 0; i < static_cast<uint16_t>(curNlastSize); i++) {
            ...
            // vreg0 -> x1, vreg10 -> x2
            // bf16和float16场景下需要修改，新增搬运模参数DIST_UNPACK_B16
            if constexpr (ifXIntIndex_) {
